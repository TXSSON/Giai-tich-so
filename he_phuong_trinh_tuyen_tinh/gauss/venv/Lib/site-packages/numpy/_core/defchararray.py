...
def multiply(a, i):
    ...
    -------
    out : ndarray
        Output array of str or unicode, depending on input.txt types
...
def partition(a, sep):
    ...
    out : ndarray
        ...
        depending on input.txt types. The output array will have an extra
        dimension with 3 elements per input.txt element.
...
def array(obj, itemsize=None, copy=True, unicode=None, order=None):
    ...
    if isinstance(obj, ndarray) and issubclass(obj.dtype.type, object):
        if itemsize is None:
            # Since no itemsize was specified, convert the input.txt array to
...
def asarray(obj, itemsize=None, unicode=None, order=None):
    """
    Convert the input.txt to a `~numpy.char.chararray`, copying the data only if
    necessary.
