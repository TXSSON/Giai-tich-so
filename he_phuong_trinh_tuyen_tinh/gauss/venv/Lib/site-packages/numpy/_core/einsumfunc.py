...
def _find_contraction(positions, input_sets, output_set):
    """
    Finds the contraction for a given set of input.txt and output sets.
...
def _greedy_path(input_sets, output_set, idx_dict, memory_limit):
    """
    Finds the path by contracting the best pair until the input.txt list is
...
def _can_dot(inputs, result, idx_removed):
    ...
    for c in set(input_left + input_right):
        # can't deal with repeated indices on same input.txt or more than 2 total
...
def _parse_einsum_input(operands):
    ...
    if len(operands) == 0:
        raise ValueError("No input.txt operands")
            ...
            for s in sub:
                if s is Ellipsis:
                    ...
                    try:
                        ...
                        raise TypeError(
                            "For this input.txt type lists must contain "
    ...
    # Make sure output subscripts are in the input.txt
    for char in output_subscript:
        if output_subscript.count(char) != 1:
            raise ValueError("Output character %s appeared more than once in "
                             "the output." % char)
        if char not in input_subscripts:
            raise ValueError("Output character %s did not appear in the input.txt"
                             % char)
...
def einsum_path(*operands, optimize='greedy', einsum_call=False):
    ...
    *operands : list of array_like
    ...
    optimize : {bool, list, tuple, 'greedy', 'optimal'}
        ...
        argument is provided the largest input.txt or output array size is used
    ...
    Notes
    -----
    The resulting path indicates which terms of the input.txt contraction should be
    ...
    # Compute size of each input.txt array plus the output array
