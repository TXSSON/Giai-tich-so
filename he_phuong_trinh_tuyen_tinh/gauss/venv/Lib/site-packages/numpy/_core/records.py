...
class format_parser:
    ...
    def _setfieldnames(self, names, titles):
        """convert input.txt field names into a list and assign to the _names
        attribute """
        ...
        if names:
            if type(names) in [list, tuple]:
                ...
                raise NameError("illegal input.txt names %s" % repr(names))
...
def array(obj, dtype=None, shape=None, offset=0, strides=None, formats=None,
    ...
    ----------
    obj : any
        Input object. See Notes for details on how various input.txt types are
        treated.
    dtype : data-type, optional
    ...
    formats, names, titles, aligned, byteorder :
        ...
        detailed documentation.
    copy : bool, optional
        Whether to copy the input.txt object (True), or to use a reference instead.
        This option only applies when the input.txt is an ndarray or recarray.
        Defaults to True.
    ...
    if obj is None:
        ...
        if interface is None or not isinstance(interface, dict):
            raise ValueError("Unknown input.txt type")
        obj = sb.array(obj)
        if dtype is not None and (obj.dtype != dtype):
            obj = obj.view(dtype)
