...
class TestDimSpec(util.F2PyTest):
    ...
    `lower` and `upper` are arbitrary expressions of input.txt parameters.
    The evaluation is performed in C, so f2py has to translate Fortran
    expressions to valid C expressions (an alternative approach is
    that a developer specifies the corresponding C expressions in a
    .pyf file).

    In the second case, when user provides an input.txt array with a given
    size but some hidden parameters used in dimensions specifications
    need to be determined based on the input.txt array size. This is a
    harder problem because f2py has to solve the inverse problem: find
    a parameter `p` such that `upper(p) - lower(p) + 1` equals to the
    size of input.txt array. In the case when this equation cannot be
    solved (e.g. because the input.txt array size is wrong), raise an
    error before calling the Fortran function (that otherwise would
    likely crash Python process when the size of input.txt arrays is
    ...
    def test_inv_array_size(self, dimspec):
        ...
        for n in [1, 2, 3, 4, 5]:
            ...
            if dimspec in self.nonlinear_dimspecs:
                # one must specify n as input.txt, the call we'll ensure
