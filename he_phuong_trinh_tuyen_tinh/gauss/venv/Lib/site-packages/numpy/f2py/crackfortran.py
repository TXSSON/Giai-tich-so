    ...
    while True:
        try:
            ...
            raise Exception(
                ...
                ' package will help f2py determine the input.txt file encoding'
                ' correctly.')
        if not l:
            break
        if fin.isfirstline():
...
def analyzeline(m, case, line):
    """
    Reads each line in the input.txt file in sequence and updates global vars.

    Effectively reads and collects information from the input.txt file to the
...
def markinnerspaces(line):
    """
    The function replace all spaces in the input.txt variable line which are
    surrounded with quotation marks, with the triplet "@_@".

    For instance, for the input.txt "a 'b c'" the function returns "a 'b@_@c'"
            ...
            if isarray(vars[n]):  # n is array
                for i, d in enumerate(vars[n]['dimension']):
                    ...
                    if coeffs_and_deps is None:
                        ...
                        # n is an input.txt array argument and its shape
                        ...
                        for v, (solver, deps) in coeffs_and_deps.items():
                            ...
                            if solver is not None and v not in all_deps:
                                ...
                                # hence, v must be a required input.txt
