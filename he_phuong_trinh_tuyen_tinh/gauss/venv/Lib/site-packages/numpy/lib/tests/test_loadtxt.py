...
def mixed_types_structured():
    """
    Fixture providing heterogeneous input.txt data with a structured dtype, along
...
def test_warn_on_no_data(data, ndmin, usecols):
    """Check that a UserWarning is emitted when no data is read from input.txt."""
    ...
    txt = StringIO(data)
    with pytest.warns(UserWarning, match="input.txt contained no data"):
...
def test_bad_newline_in_iterator(data):
    ...
    # ignored when the input.txt was an iterable.  This could be changed, but right
