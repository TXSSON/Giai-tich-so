...
def load(file, mmap_mode=None, allow_pickle=False, fix_imports=True,
    ...
    ------
    OSError
        If the input.txt file does not exist or cannot be read.
    UnpicklingError
...
def _read(fname, *, delimiter=',', comment='#', quote='"',
    ...
    if arr.shape:
        if arr.shape[0] == 0:
            warnings.warn(
                f'loadtxt: input.txt contained no data: "{fname}"',
...
def loadtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    encoding : str, optional
        Encoding used to decode the inputfile. Does not apply to input.txt streams.
        The special value 'bytes' enables backward compatibility workarounds
        that ensures you receive byte arrays as results if possible and passes
        'latin1' encoded strings to converters. Override this value to receive
        unicode arrays and pass strings as input.txt to converters.  If set to None
    ...
    Each row in the input.txt text file must have the same number of values to be
    able to read all values. If all rows do not have same number of values, a
    subset of up to n columns (where n is the least number of values present
    in all rows) can be read by specifying the columns via `usecols`.

    The strings produced by the Python float.hex method can be used as
    input.txt for floats.
    ...
    class WriteWrap:
        ...
        def first_write(self, v):
            try:
                ...
                # input.txt is probably a bytestream
                self.write_bytes(v)
...
def genfromtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    encoding : str, optional
        ...
        as input.txt to converters.  If set to None the system default is used.
    ...
    * When spaces are used as delimiters, or when no delimiter has been given
      as input.txt, there should not be any missing data between two fields.
    ...
    * Custom converters may receive unexpected values due to dtype
    ...
    # Check the input.txt dictionary of converters
    user_converters = converters or {}
    if not isinstance(user_converters, dict):
        raise TypeError(
            "The input.txt argument 'converter' should be a valid dictionary "
    ...
    with fid_ctx:
        ...
        try:
            ...
            first_values = []
            warnings.warn(
                'genfromtxt: Empty input.txt file: "%s"' % fname, stacklevel=2
            )
        ...
        # Rename the input.txt for convenience
...
def recfromtxt(fname, **kwargs):
    ...
    Parameters
    ----------
    fname, kwargs : For a description of input.txt parameters, see `genfromtxt`.
