...
def ix_(*args):
    ...
    out : tuple of ndarrays
        N arrays with N dimensions each, with N the number of input.txt
...
class OGridClass(nd_grid):
    ...
    mesh-grid : ndarray or tuple of ndarrays
        If the input.txt is a single slice, returns an array.
        If the input.txt is multiple slices, returns a tuple of arrays, with
...
def fill_diagonal(a, val, wrap=False):
    ...
    modifies the input.txt array in-place without returning a value.
    ...
    if a.ndim == 2:
        ...
        if not np.all(diff(a.shape) == 0):
            raise ValueError("All dimensions of input.txt must be of equal length")
...
def diag_indices_from(arr):
    ...
    if not arr.ndim >= 2:
        raise ValueError("input.txt array must be at least 2-d")
    # For more than d=2, the strided formula is only valid for arrays with
    # all dimensions equal, so we check first.
