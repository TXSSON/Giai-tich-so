...
def iscomplex(x):
    """
    Returns a bool array, where True if input.txt element is complex.

    What is tested is whether the input.txt has a non-zero imaginary part, not if
    the input.txt type is complex.
...
def isreal(x):
    """
    Returns a bool array, where True if input.txt element is real.
    ...
    Returns True for all elements in input.txt array of ``dtype=object`` even if
...
def iscomplexobj(x):
    ...
    The type of the input.txt is checked, not the value. Even if the input.txt
    has an imaginary part equal to zero, `iscomplexobj` evaluates to True.

    Parameters
    ----------
    x : any
        The input.txt can be of any type and shape.
...
def isrealobj(x):
    ...
    The type of the input.txt is checked, not the value. So even if the input.txt
    ...
    accepts all other objects. Since it assumes array input.txt, the return
...
def real_if_close(a, tol=100):
    """
    If input.txt is complex with all imaginary parts close to zero, return
    real parts.
...
def typename(char):
    ...
    -------
    out : str
        Description of the input.txt data type code.
...
def common_type(*arrays):
    """
    Return a scalar type which is common to the input.txt arrays.
    ...
    All input.txt arrays except int64 and uint64 can be safely cast to the
