...
def nanmin(a, axis=None, out=None, keepdims=np._NoValue, initial=np._NoValue,
    ...
    If the input.txt has a integer type the function is equivalent to np.min.
...
def nanmax(a, axis=None, out=None, keepdims=np._NoValue, initial=np._NoValue,
    ...
    If the input.txt has a integer type the function is equivalent to np.max.
...
def nanargmin(a, axis=None, out=None, *, keepdims=np._NoValue):
    ...
    a : array_like
        Input data.
    axis : int, optional
        Axis along which to operate.  By default flattened input.txt is used.
    out : array, optional
...
def nanmean(a, axis=None, dtype=None, out=None, keepdims=np._NoValue,
    ...
    dtype : data-type, optional
        ...
        is `float64`; for inexact inputs, it is the same as the input.txt
        dtype.
    out : ndarray, optional
        Alternate output array in which to place the result.  The default
        is ``None``; if provided, it must have the same shape as the
    ...
    Note that for floating-point input.txt, the mean is computed using the same
...
def nanstd(a, axis=None, dtype=None, out=None, ddof=0, keepdims=np._NoValue,
    ...
    For floating-point input.txt, the *std* is computed using the same
    precision the input.txt has. Depending on the input.txt data, this can cause
    the results to be inaccurate, especially for float32 (see example
    below).  Specifying a higher-accuracy accumulator using the `dtype`
    keyword can alleviate this issue.
