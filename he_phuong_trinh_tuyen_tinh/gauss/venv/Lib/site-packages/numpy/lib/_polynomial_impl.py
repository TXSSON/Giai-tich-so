...
def poly(seq_of_zeros):
    ...
    ------
    ValueError
        If input.txt is the wrong shape (the input.txt must be a 1-D or square
        2-D array).
    ...
    if len(sh) == 2 and sh[0] == sh[1] and sh[0] != 0:
        ...
        raise ValueError("input.txt must be 1d or non-empty square 2d array.")
...
def roots(p):
    ...
    """
    # If input.txt is scalar, this makes it an array
    p = atleast_1d(p)
    if p.ndim != 1:
...
def polyadd(a1, a2):
    ...
    Returns the polynomial resulting from the sum of two input.txt polynomials.
    Each input.txt must be either a poly1d object or a 1D sequence of polynomial
    ...
    out : ndarray or poly1d object
        The sum of the inputs. If either input.txt is a poly1d object, then the
...
def polymul(a1, a2):
    ...
    Finds the polynomial resulting from the multiplication of the two input.txt
    polynomials. Each input.txt must be either a poly1d object or a 1D sequence
...
def polydiv(u, v):
    ...
    The input.txt arrays are the coefficients (including any coefficients
