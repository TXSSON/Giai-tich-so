...
def consistent_subclass(out, in_):
    # For ndarray subclass input.txt, our output should have the same subclass
    # (non-ndarray input.txt gets converted to ndarray).
...
class TestSVD(SVDCases, SVDBaseTests):
    def test_empty_identity(self):
        """ Empty input.txt should put an identity matrix in u or vh """
        x = np.empty((4, 0))
...
class _TestNormBase:
    ...
    def check_dtype(x, res):
        if issubclass(x.dtype.type, np.inexact):
            ...
            # For integer input.txt, don't have to test float precision of output.
...
def test_cross():
    ...
    with assert_raises_regex(
        ValueError,
        r"input.txt arrays must be \(arrays of\) 3-dimensional vectors"
    ):
        x_2dim = x[:, 1:]
