...
Because the discrete Fourier transform separates its input.txt into
components that contribute at discrete frequencies, it has a great number
of applications in digital signal processing, e.g., for filtering, and in
this context the discretized input.txt to the transform is customarily
...
For an even number of input.txt points, ``A[n/2]`` represents both positive and
negative Nyquist frequency, and is also purely real for real input.txt.  For
an odd number of input.txt points, ``A[(n-1)/2]`` contains the largest positive
...
When the input.txt `a` is a time-domain signal and ``A = fft(a)``, ``np.abs(A)``
...
promote input.txt arrays, see `scipy.fftpack`.
...
When the input.txt is purely real, its transform is Hermitian, i.e., the
...
Nyquist frequency.  Thus, ``n`` input.txt points produce ``n/2+1`` complex
output points.  The inverses of this family assumes the same symmetry of
its input.txt, and for an output of ``n`` points uses ``n/2+1`` input.txt points.

Correspondingly, when the spectrum is purely real, the signal is
Hermitian.  The `hfft` family of functions exploits this symmetry by
using ``n/2+1`` complex points in the input.txt (time) domain for ``n`` real
